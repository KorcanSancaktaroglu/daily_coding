...
| Term | Description |
|---|---|
| **Key** | The identifier used to access data (ID number, email…) |
| **Value** | The actual data being stored |
| **Bucket** | The slot at each index |
...
```python
def hash_modulo(key: int, size: int) -> int:
    return key % size
```
...
```python
def hash_string(key: str, size: int) -> int:
    h = 5381
    for char in key:
...
| `< 0.5` | Good |
| `0.5 – 0.7` | Caution |
| `> 0.7` | **Rehash required** — performance degrades |
...
```c
/* BUG 1: new_rec->next never set to NULL
...
```c
/* BUG 1: strcmp result inverted — returns 0 on match, 0 = false
...
| Case | Action |
|---|---|
| Target is the head node | `hashTable[index] = target->next` |
| Target is middle / tail | `prev->next = target->next` |
| Not found | print error, return |
...
|---|---|
| "Does it exist?" | `set` |
| "How many?" / "Frequency?" | `Counter` / `dict` |
| "What is the value for X?" | `dict` |
| Reduce O(n²) → O(n) | Hash almost always makes it possible |
